...
var permissionCatalog = []struct {
	...
	{"Intelligence", "intelligence:read", "Intelligence 조회", "Data Scope 범위의 Signal, Risk, Insight, 추천 행동을 조회합니다."},
	{"Intelligence", "intelligence:write", "추천 행동 처리", "추천을 수락해 Task로 전환하거나 무시하고, Risk 감수와 Signal 무시를 기록합니다."},
	{"Intelligence", "intelligence:run", "Intelligence 분석 실행", "규칙 엔진을 즉시 실행합니다. 전체 고객을 훑으므로 관리자용입니다."},
	...
	{"고객의 목소리", "voice:knowledge-review", "지식 반영 판정", "종결된 요청을 에이전트 진단 근거로 쓸지(반영·제외) 판정합니다. 담당자와 분리된 검토자 전용 권한입니다."},
	...
	{"관리", "analytics:manage", "방문자 분석 설정", "추적 스크립트와 허용 출처를 설정합니다. 모든 사용자의 보안 정책이 넓어지므로 일반 관리자 권한과 분리됩니다."},
